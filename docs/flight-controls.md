...
mouse-X (ED default), roll on Q/E (keys)**, lateral thrust on **Z/C**. This puts the
...
| **Lateral thrust L/R**                | Z / C                                                            | ED strafe (preset-dependent)                                                | **Hold-to-fire** momentary impulse; keep cheap for surface combing.                                                                                                                                                           |
| **Vertical thrust up/down**           | Space / Ctrl                                                     | ED default                                                                  | Hold-to-fire impulse.                                                                                                                                                                                                         |
| **Yaw (keyboard, optional)**          | A / D                                                            | ED keyboard alt                                                             | A/D are freed up by yaw-on-mouse; optional fine/trim yaw for keyboard-only nudges. Leave unbound if it muddies the layout.                                                                                                    |
...
| **Stabilize (the assist)**            | Z                                                                | ED `Z` = FA toggle / hold-to-disable                                        | **Inverted semantics:** costed drift-kill, not a free baseline. Current Stabilizer is momentary, so v1 is a **tap**, not a held mode. See §4.                                                                                 |
...
- **`X`/`Z` to stop does nothing.** ED's `X` stops you only because FA bleeds the residual. Zeroing a non-existent setpoint visibly does nothing — **bind `X`/`Z` to the Stabilizer** and teach "tap to bleed velocity; it costs heat and can lock out." There is **no free all-stop**; say so.
...
4. **Keyboard yaw on A/D — keep or drop?** Yaw now lives on mouse-X; A/D are free. Optional trim yaw vs. leaving them unbound.
